...
import { Mail, FileText, Share2, Link, Check } from 'lucide-react';
...
async function generateAndShareStory(destinations: Destination[], vibe: string) {
  ...
  top3.forEach((d) => {
    ...
    // Why it fits — short italic line to fill space
    if (d.whyItFits) {
      y += 4;
      ctx.font = '300 22px system-ui, sans-serif';
      ctx.fillStyle = 'rgba(255,255,255,0.52)';
      const whyBottom = wrapText(ctx, d.whyItFits, INNER_X, y, INNER_W, 32, 'left');
      y = whyBottom + 24;
    }
    ...
    // Best time on its own line below — no truncation
    y += 30;
    ctx.fillStyle = 'rgba(255,255,255,0.38)';
    ctx.font = '300 21px system-ui, sans-serif';
    wrapText(ctx, `📅 ${d.bestTime}`, INNER_X, y, INNER_W, 30, 'left');
...
export default function ExportBar({ destinations, vibe }: Props) {
  ...
  const copyLink = async () => {
    const url = `${window.location.origin}/?v=${encodeURIComponent(vibe)}`;
    try {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', padding: '24px 0 8px' }}>
      ...
      <button
        style={{ ...btn, ...(copied ? { borderColor: '#4ade80', color: '#4ade80' } : {}) }}
        onClick={copyLink}
        onMouseEnter={e => !copied && hover(e, true)}
        onMouseLeave={e => !copied && hover(e, false)}>
